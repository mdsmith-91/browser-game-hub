const SnakeGame = (() => {
  ...
  function endRound(deaths) {
    active = false; clearInterval(timer); playTimer.stop(); let message;
    ...
    else { Storage.saveHighScore('snake', scores[0]); Storage.recordResult('snake', 'complete'); message = `Game over. Score: ${scores[0]}`; }
  ...
  function updateUI(message) { const stats = Storage.getStats('snake'); document.getElementById('score').textContent = scores[0]; document.getElementById('player-two-score').textContent = mode === 'two' ? scores[1] : '-'; document.getElementById('high-score-display').textContent = Storage.getHighScore('snake') || 0; document.getElementById('level').textContent = level; document.getElementById('speed-display').textContent = level > 1 ? 'Faster' : 'Normal'; document.getElementById('games-played').textContent = stats.gamesPlayed; document.getElementById('wins').textContent = stats.wins; document.getElementById('losses').textContent = stats.losses; document.getElementById('draws').textContent = stats.draws; document.getElementById('time-played').textContent = `${Math.floor(stats.timePlayed / 60)}m ${stats.timePlayed % 60}s`; document.getElementById('snake-status').textContent = message || (mode === 'two' ? 'Two-player Snake is keyboard recommended: Player 1 uses WASD; Player 2 uses arrow keys.' : 'Swipe the board, use the D-pad, or use arrows/WASD to move.'); }
  ...
  function cleanup() { active = false; clearInterval(timer); playTimer.stop(); document.removeEventListener('keydown', keydown); }
