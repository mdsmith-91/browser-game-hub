const SnakeGame = (() => {
  ...
  function updateUI(message) { document.getElementById('score').textContent = scores[0]; document.getElementById('player-two-score').textContent = mode === 'two' ? scores[1] : '-'; document.getElementById('high-score-display').textContent = Storage.getHighScore('snake') || 0; document.getElementById('level').textContent = level; document.getElementById('speed-display').textContent = level > 1 ? 'Faster' : 'Normal'; document.getElementById('snake-status').textContent = message || (mode === 'two' ? 'Simultaneous play: Player 1 uses WASD; Player 2 uses arrow keys.' : 'Use arrows or WASD to move.'); }
